...
(defun floating-hex-c-readable (number)
  (floating-hex-to-c-readable (format nil "~16/floating/" number)))
...
(defun parse-c-readable (string)
  "Given a STRING as output by '(floating-hex-to-c-readable (format nil \"~16/floating/\" NUMBER))', or as output by 'printf(\"%a\", NUMBER)' in C, return the floating-point number encoded by that string.
...
(defun test-parse-c-readable ()
  (let ((numbers '(-1024.0 -4.0 -3.0 -2.0 -1.0 -0.4 -0.3 -0.2 -0.1 0.0 0.1 0.2 0.3 0.4 1.0 2.0 3.0 4.0 1024.0))
    ...
    (loop
       ...
       do
	 (let ((n1 (parse-c-readable hex-floating-number))
	       (n2 (parse-c-readable printf-number)))
	   (assert (= number n1 n2))))))

(defun test-parse-c-readable-2 (&optional (tests 1000))
  (loop for i below tests do
       (let* ((number (* (if (= 0 (random 2)) -1 1) (random 1e37)))
	      (hex-floating-number (floating-hex-c-readable number)))
	 (let ((number-parsed (parse-c-readable hex-floating-number)))
