(defparameter *lisp-symbols*
  '(&allow-other-keys
    ...
    *trace-output*
    ...
    yes-or-no-p
    zerop)
  "All 978 Common Lisp symbols taken from the CLHS 'Alphabetical Symbol Index (Full)' in 'Front/X_AllSym.htm'")

(defparameter *lisp-special-operators*
  (remove-if (complement #'special-operator-p) *lisp-symbols*)
  "Common Lisp symbols which are special operators.")

(defparameter *lisp-macros*
  (remove-if (complement #'macro-function) *lisp-symbols*)
  "Common Lisp symbols for which a macro-function is defined.")

(defparameter *lisp-compiler-macro-functions*
  (remove-if (complement #'compiler-macro-function) *lisp-symbols*)
  "Common Lisp symbols for which a compiler-macro-function is defined.")

;; TODO: I'm not sure this excludes all symbols not describing a "normal" function. I couldn't find a way to determine whether the returned value of (SYMBOL-FUNCTION 'CAR) is of the form #<FUNCTION ...> or 
(defparameter *lisp-functions*
  (remove-if (lambda (s) (not (and (fboundp s) (not (macro-function s)) (not (special-operator-p s))))) *lisp-symbols*)
  "Common Lisp symbols which are normal functions.")

(defparameter *lisp-functions-hack*
