;; TODO: Try to find chunks of instruction sequences that are popular. This could be done by dropping the whole genome sequence of a dead organism on the floor, where it decays slowly. Whenever an organism walks over that coordinate, it may pick up one of the chunks and incorporate it in its currently produced offspring. The goal is to simulate molecules in reality, which are getting more and more complex over time, with more and more chemical energy (obtained by photosynthesis or by eating plants) having been stored in the bounds between the atoms.

;; TODO: Sexual reproduction: Try recombining mutations that developed around the same time. (To make it independent of the sexual recombination implementation, maybe record the world tick at which each instruction was inserted into the organism's genes.)
