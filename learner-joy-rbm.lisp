    ...
    (let* ((codes (generate-joy-codes n stk joy-ops 'nest 'unnest max-ticks max-seconds stk-length exp-length res-length))
      ...
      (flet ((get-data (iteration rbm)
		 ...
		 (when (= 0 (rem iteration 10))
		   (let* ((decode (map 'list (lambda (code) (decode-joy-input-and-result code joy-ops 'nest 'unnest stk-length exp-length res-length)) data))
			  (decode-1 (car decode)))
...
;; few joy-ops:
;;(defparameter *rbm* (learn-joy-step 1 '(5.5) '(pred succ) 1000 .01 2 2 2 12 '(:h-softmax nil :h-binary 12 :h-gaussian 0) 2 .1 .9 .0002 3000))
;;(defparameter *rbm* (learn-joy-step 1 '(5.5) *joy-ops* 1000 .01 2 2 2 40 '(:h-softmax nil :h-binary 40 :h-gaussian 0) 1 .1 .9 .0002 3000))

(let* (;(joy-ops *joy-ops*)
       (joy-ops '(succ pred pop dup nill true))
       (stk-length 2)
       (exp-length 2)
       (res-length 2)
       (c (generate-joy-codes 1 '(5.5) joy-ops 'nest 'unnest 1000 0.01 stk-length exp-length res-length)))
  (defparameter *data-codes* (make-array (list (length c) (length (car c))) :element-type 'single-float :initial-contents c))
  (multiple-value-bind (v v-types) (rbm-input-parameters-joy-input-and-result joy-ops stk-length exp-length res-length)
    (print (list v v-types))))
;;(defparameter *rbm* (rbm-learn-minibatch *data-codes* (new-rbm 73 73 :v-binary 1 :v-softmax '(11 11 11 11 11 11) :v-gaussian 6 :h-binary 73) .01 .9 .0002 500))
;;(reconstruct-joy-step '(5.5) '(succ) nil '(succ pred pop dup nill true) 2 2 2 *rbm*)
