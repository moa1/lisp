;; This paragraph is a short summary of my understanding of forwards and backwards inferencing. Consider the following program (taken from the NTI paper): (PROGN (SETQ X (SQRT Y)) (IF (> X 0) 1 -1)). (And, for this example, lets assume our type lattice only consists of '(T COMPLEX REAL INTEGER BOOL).). In forwards inferencing, we learn about the type of a function result from the types of the arguments of the function. In backwards inferencing, we learn about the type of a function argument from the type of the result of the function. So in the first forwards pass, we learn that X has type NUMBER, because it is set to the result of #'SQRT, and the join of the result of all pre-defined #'SQRT is NUMBER. We do not learn anything else about the variables X or Y in the first forwards pass. In the first backward pass, we learn that X must be a REAL, because #'> is not defined for COMPLEX numbers. Then, also in the same backward pass, we learn that the result of (SQRT Y) must be a REAL, because X is set to the result, and X is REAL. In addition (in the same backward pass), we learn that Y must be REAL, because if the result of a #'SQRT-call is a REAL, then the argument must have been a REAL. Do we really need two different functions for forwards and backwards passes? We could put forwards and backwards inferencing into the same, single, method #'INFER. For example, (INFER PROGN-FORM) would call (INFER SETQ-FORM) and then (INFER IF-FORM). The (INFER SETQ-FORM) would call (INFER APPLICATION-FORM of #'SQRT), which would return type COMPLEX, which would be assigned to X. The (INFER IF-FORM) would call (INFER APPLICATION-FORM of #'>). This (INFER APPLICATION-FORM of #'>) would look up the possible pre-defined types for #'>, when the first argument is a subtype of COMPLEX (because X is COMPLEX) and the second argument is a subtype of INTEGER (because 0 is an INTEGER). It would find (> REAL INTEGER) = BOOL, and (> INTEGER INTEGER) = BOOL. So (INFER APPLICATION-FORM of #'>) would return the type BOOL, but it would also do its part of backwards inferencing, which is to set the first argument X to the join of REAL and INTEGER (#'MEET-ed with the previously known type of X, which is REAL), which is REAL. It would set this in the X of the form computed by (LAST-SETQ X), which is the SETQ-FORM of the program (i.e. the first PROGN-FORM). After assigning the join of 1 and -1 (INTEGER) to the IF-FORM, and then the PROGN-FORM, the first #'INFER-pass would be finished. The second #'INFER-pass would call (INFER PROGN-FORM), which would call (INFER SETQ-FORM). This would assign the type of X (namely REAL), to the result of the APPLICATION-FORM of #'SQRT, and call (INFER APPLICATION-FORM of #'SQRT). (INFER APPLICATION-FORM of #'SQRT) would look up the pre-defined functions #'SQRT, and remove from this list (1.) the #'SQRTs whose first argument is not a subtype of T, and (2.) the #'SQRTs whose result is not a subtype of REAL. The filtered list then only consists of (SQRT REAL) = REAL. So (INFER APPLICATION-FORM of #'SQRT) could assign the result of the APPLICATION-FORM to the #'MEET of REAL and REAL (which is REAL), and the type of Y to the join of the types of all first arguments of the filtered list, which is (JOIN REAL), which is REAL. Further passes would not be able to further restrict the type bounds. So (INFER APPLICATION-FORM of #'F) has to (1.) look up the pre-defined functions for #'F and filter them according to the known types of result and arguments, (2.) compute, for each I-th argument, the join of the I-th argument in the filtered list, meet this join with the known result of the I-th argument, and assign this meet to the result of the I-th argument, and (3.) compute the join of the results, meet it with the known result, assign this meet to the result and return it. All other forms have to take care not to overwrite any result, but MEET it with the already known result type. So we do not need a function for each of forwards and backwards pass, if the one, combined function #'INFER propagates the found result of a variable back to the last setq of the variable, and a later pass propagates the restricted type further and further up to earlier forms. But probably the computation would be faster if a separate #'BKW-INFER (backward inference) function would visit the forms in reverse-eval order, and thus propagate type quicker to earlier forms.
;; For example, consider the program (LET (B C D) (SETQ B (1+ A)) (SETQ C (1+ B)) (SETQ D (1+ C)) (ASSERT (TYPEP D INTEGER))). A combined function #'INFER would need 4 passes to learn the type of the variables A, B, C, and D (one for each variable). Separate #'FWD-INFER and #'BKW-INFER would need 1 forward pass and 1 backward pass. Maybe I can separate control flow from inference. Control flow could be done by #'FIND-EXITS, and call a function #'STORE-ORDER on each visited form, in the order they are evaluated in. #'STORE-ORDER only stores the order of the forms it is called with. The main inference program then calls #'INFER on the list of stored forms, alternatingly in forward and reverse order. #'INFER on an AST does not call #'INFER on its sub-ASTs, but instead only stores the known type bounds in the RESULT-slots of the sub-forms. Let's think this through on the program (LET ((A 1) B) (TAGBODY S (IF (= A 10) (GO E)) (SETQ B (+ A A) A (1+ A)) (GO S) E) B). #'FIND-EXITS would first visit the OBJECT-FORM (for 1), then the VAR-BINDINGs for A and B, then the subforms of the TAGBODY-FORM, which would first visit their sub-forms, then the TAGBODY-FORM, the VAR-READING of B, and then the LET-FORM. An invocation (INFER OBJECT-FORM of 1) would set the RESULT-slot of the OBJECT-FORM to the appropriate type (i.e. INTEGER for 1), (INFER VAR-BINDING of X) would look up the RESULT-slot of its VALUE-slot, i.e. the OBJECT-FORM, meet this with the known type bounds of X, and finally store that result in the RESULT-slot of the VAR-BINDING. The sub-forms of the sub-forms of the TAGBODY-FORM would look up the types of their arguments using #'LAST-SETQ, use these types to compute their result, meet that result with the known RESULT-slot, and finally store that new result in their RESULT-slot. For example, #'FIND-EXITS would break down the form (IF (= A 10) (GO E)) into the order A, 10, (= A 10), (GO E), (IF (= A 10) (GO E)). (INFER A) would use (LAST-SETQ A) to look up the updated type of A, meet it with its known and old type, and store that result in its RESULT-slot. (INFER 10) doesn't just store INTEGER into its RESULT-slot, but checks whether the value of the RESULT-slot (which another form may have updated) is a subtype of INTEGER, and if not, signals an error. (INFER (= A 10)) looks up the up-to-date RESULT-slots of the arguments of #'=, namely A and 10, looks up #'=, filters with their arguments' types, joins the results of the filtered list of #'=, meets it with the known RESULT, and stores that result in the RESULT-slot for (= A 10). If #'INFER does forwards and backwards inference in one go, it also updates the RESULT-slots of its arguments from the filtered list of #'=, as described above (search for #'SQRT). (INFER (GO E)) always sets its RESULT-slot to (MAKE-BOUNDS (MAKE-RESULTS-0) (MAKE-RESULTS-0)). (INFER (IF (= A 10) (GO E))) looks up the RESULT-slot of its THEN- and ELSE-branch, joins them, meets that with the known RESULT-slot (of the IF-FORM), and stores the new result in its RESULT-slot. This finishes the forwards inference of the IF-FORM. Then (INFER SETQ-FORM) would be ordered analogously, into A, A, (+ A A), (VAR-WRITING of B to (+ A A)), A, (1+ A), (VAR-WRITING of A to (1+ A)), (SETQ B (+ A A) A (1+ A)). #'INFER on VAR-READINGs and APPLICATION-FORMs has been described. (INFER VAR-WRITING) would update its own RESULT-slot (where updating means checking for incompatible known and new type bounds). (INFER SETQ-FORM) would update (*(and check) its RESULT-slot from the last VAR-WRITING. (INFER TAGBODY-FORM) just checks whether its last form has normal exits, and stores the type NULL if yes, and the type (MAKE-BOUNDS (MAKE-RESULTS-0) (MAKE-RESULTS-0)) if not. The main inference program then calls #'INFER on the forms in reverse, backwards, order, thereby updating the bounds of the RESULT-slots. Then in forwards order again, backwards again, etc.. This continues until the RESULT-slots do not change anymore.
...
(defparameter +builtin-types+
  '(nil t null boolean list ;these are always needed: NIL and T as bottom and top element of the type lattice, NULL as result of TAGBODY, BOOLEAN as type of &OPTIONAL and &KEY suppliedp parameters, and LIST for &REST parameters.
    number integer fixnum unsigned-byte real single-float (and unsigned-byte fixnum)
    symbol
    v4s-float
...
(defun test-meet-join-results ()
  ...
  (assert (is-results (join-results (make-results 'fixnum) (make-results 'single-float)) (make-results 'real)))
...
For copies of WALKER:FUN-BINDINGs, this stores the original WALKER:FUN-BINDING.")
...
#|      (loop for name in variables do
      ...
      (loop for name in functions do
	   (multiple-value-bind (arglist decls values-type) (parse-builtin-lisp-function-decls name)
	     (multiple-value-bind (parsed-llist llist-parser)
		 (walker:parse-ordinary-lambda-list parser arglist nil)
	       (let* ((parsed-declspecs (walker:parse-declspecs llist-parser (cdr decls) nil))
		 ...
		 (setf (walker:form-parent parsed-llist) definition)
...
(defmethod walker:deparse :around ((deparser deparser-annotate) (ast walker:application-form) path)
  (let ((annotated-ast (call-next-method))
	(annotated-funbinding (cond
				((form-fun-binding ast)
				 (walker:deparse deparser (form-fun-binding ast) nil))
				((typep (walker:nso-definition (walker:form-fun ast))
					'builtin-functiondef)
				 :is-builtin)
				(t
				 :is-nil))))
...
;; TODO: think about a macro that unifies checking for recursive calls, in the same manner as macro #'WITH-EXITS, so that we hopefully don't need a CALLSTACK-slot in all of the classes EXIT-FINDER, ACCESSES-FINDER, LAST-SETQS-FINDER, and FWD-INFERER, but only in EXIT-FINDER, which would then be responsible for finding exits and recursive control flow.
    ...
    (cond
      ...
      (t
       ...
       (let* ((llist (walker:form-llist funbinding))
	      (parser (make-instance 'ntiparser)) ;FIXME? needed by ARGUMENTS-ASSIGN-TO-LAMBDA-LIST
...
(defmethod find-accesses ((finder accesses-finder) (ast walker:var-bindings-mixin))
  (let* ((parser (make-instance 'ntiparser))
	 (init-values (loop for binding in (walker:form-bindings ast) collect
			   (let ((value (walker:form-value binding)))
			     (if (null value) (walker:make-nil parser binding) value))))
	 (read nil)
	 (written nil))
...
(defmethod last-setqs ((finder last-setqs-finder) (ast walker:application-form) var last-setqs)
  (declare (optimize (debug 3)))
  (let ((fun-definition (walker:nso-definition (walker:form-fun ast))))
    (cond
      ((and (not (typep fun-definition 'builtin-functiondef))
      ...
      (t
       ...
       (let* ((parser (finder-parser finder))
	      ...
	      (fun-binding (if (typep fun-definition 'builtin-functiondef)
			       fun-definition
			       (form-fun-binding ast)))
	 ...
	 (loop for acons in alist do
	    ;; TODO: when implementing variable (value) bindings: bind result to (CAR ACONS).
	      (let ((initforms (let ((initform (cdr acons)))
				 (if (consp initform) ;to handle &REST parameter
				     initform
				     (list initform)))))
		(loop for initform in initforms do
		     (last-setqs-form! finder initform var last-setqs
		       (pop (finder-callstack finder))
		       (return-from last-setqs last-setqs)))))
	 (unless (typep fun-definition 'builtin-functiondef)
...
(defun test-last-setqs-form (form &key (captured-symbol 'var) (warn-multiple-captures t) variables functions)
...
(defun test-last-setqs ()
  (flet ((assert-result (form desired-last-setqs &key variables functions)
	   (let* ((last-setqs (test-last-setqs-form form :warn-multiple-captures nil :variables variables :functions functions))
		  (actual-last-setqs (remove-duplicates (mapcar (lambda (form) (walker:deparse (make-instance 'walker:deparser) form nil)) (mapcar #'walker:form-value last-setqs)))))
    ...
    ;;(assert-result '(let ((a 1)) (labels ((f (&optional (a (setq a 2))) (if 1 (f) a) (f))) (f)) a) '()) ;recurses forever, therefore I'm not sure there is a correct last setq
    (assert-result '(let ((a 1)) (labels ((f (&optional (a (setq a 2))) (if 1 (f) a))) (f)) a) '(2))
    ...
    (assert-result '(let ((a (1+ 1))) (1+ a) a) '((1+ 1)) :functions '(1+))
    (assert-result '(let ((a (1+ 1))) (1+ (setq a 2)) a) '(2) :functions '(1+))
...
(defun setq-forms-for-var-reading (inferer var-reading)
  (let* ((last-setqs (last-setqs-for-var-reading (inferer-last-setqs-finder inferer) var-reading))
	 (setq-forms (mapcar (lambda (setq)
			     ...
			     last-setqs)))
    (assert (not (null last-setqs)) () "No LAST-SETQS for ~S." var-reading)
    setq-forms))

(defun bounds-of-var-reading (fwd-inferer var-reading)
  (let* ((setq-bounds (setq-forms-for-var-reading fwd-inferer var-reading)))
    ;; TODO: have to meet bounds with (all) declared types.
    (apply #'join-bounds (mapcar #'form-bounds setq-bounds))))
...
(defun meet-forms (branches ast &key (ast-bounds (form-bounds ast)))
  (mapc (lambda (form)
	  (let* ((met-bounds (meet-bounds (form-bounds form) ast-bounds)))
	    (check-new-bounds form met-bounds)
	    (setf (form-bounds form) met-bounds)))
	branches))

(defmethod infer ((inferer inferer) (ast walker:var-reading))
  (let ((setq-forms (setq-forms-for-var-reading inferer ast)))
    ;; TODO: have to meet bounds with (all) declared types.
...
(defun infer-builtin-functiondef (application-form funbinding)
  "Does the forward and backward pass on the built-in Lisp function FUNBINDING."
  (declare (type walker:application-form application-form) (type builtin-functiondef funbinding))
  (declare (optimize (debug 3)))
  (labels ((warn-argument-parameter-type (argument-form parameter-type)
	     (flet ((warn-resultn (n parameter-type)
		      (let* ((form-bounds (form-bounds argument-form))
			     (argument-upper (resultn (bounds-upper form-bounds) n))
			     (argument-lower (resultn (bounds-lower form-bounds) n)))
			(unless (is-subtypep argument-upper parameter-type +builtin-typehash+)
			  (warn "Function parameter upper bounds type mismatch: ~S should be a subtype of ~S." argument-upper parameter-type))
			(unless (is-subtypep nil argument-lower +builtin-typehash+)
			  (warn "Function parameter lower bounds type mismatch: ~S should be a subtype of ~S." nil argument-lower)))))
	       (cond
		 ((consp parameter-type)
		  (assert (eql (car parameter-type) 'values))
		  (loop for type in (cdr parameter-type) for n from 0 do (warn-resultn n type)))
		 (t
		  (warn-resultn 0 parameter-type)))))
	   (process-arguments-parameters (process-function)
	     (let* ((llist (walker:form-llist funbinding))
		    (parser (make-instance 'ntiparser)) ;FIXME? needed by ARGUMENTS-ASSIGN-TO-LAMBDA-LIST
		    (arguments (walker:form-arguments application-form))
		    (arg-alist (walker-plus:arguments-assign-to-lambda-list parser llist arguments)))
	       (loop for acons in arg-alist for acons-rest on arg-alist do
		    (let* ((var (car acons))
			   (types (remove-if (lambda (declspec) (not (typep declspec 'walker:declspec-type)))
					     (walker:nso-declspecs var)))
			   (type (join-typelist (mapcar #'walker:declspec-type types) +builtin-typehash+))
			   (parameter (walker:nso-definition var))
			   (form (cdr acons)))
		      (funcall process-function parameter form type))))))
    ;; forward pass: from the matching ARGUMENTS, find a built-in function, and set the APPLICATION-FORM.
    (process-arguments-parameters
     (lambda (parameter argument-form parameter-type)
       ;; forward pass: check that the bounds of FORM are a subtype of the declaration of PARAMETER.
       (etypecase parameter
	 (walker:required-parameter (warn-argument-parameter-type argument-form parameter-type))
	 (walker:optional-parameter (warn-argument-parameter-type argument-form parameter-type))
	 (walker:rest-parameter
	  ;; TODO: when I have implemented algebraic types, declare types of the PARAMETER in +BUILTIN-LISP-FUNCTIONS+ like so (DECLARE (FTYPE (FUNCTION (&REST (LIST NUMBER NUMBER)) NUMBER) +)) and match the elements of FORM with the declared &REST types (i.e. (LIST NUMBER NUMBER)).
	  )
	 (walker:key-parameter (warn-argument-parameter-type argument-form parameter-type)))))
    ;; forward pass: update the APPLICATION-FORMs result from the value type of the declaration.
    (let* ((decl-result-bounds (walker:form-values funbinding)))
      (join-forms application-form nil :branches-bounds (list decl-result-bounds))
      ;; backward pass: from the result value, find a matching function and update the arguments' types from the parameters. But first warn if the result type does not match the declared result type.
      (let ((result-values (cons 'values
				 (let* ((u (bounds-upper decl-result-bounds))
					(l (bounds-lower decl-result-bounds))
					(m (max (results-nvalues u) (results-nvalues l))))
				   (loop for i below (most-significant-bit m) collect (resultn u i))))))
	(warn-argument-parameter-type application-form result-values)))
    (process-arguments-parameters
     (lambda (parameter argument-form parameter-type)
       ;; backward pass: update the arguments' types from the parameters.
       (flet ((meet-non-list-type ()
		(let* ((upper (make-results parameter-type))
		       (parameter-bounds (make-bounds upper (make-results-nil))))
		  (meet-forms (list argument-form) nil :ast-bounds parameter-bounds))))
	 (etypecase parameter
	   (walker:required-parameter (meet-non-list-type))
	   (walker:optional-parameter (meet-non-list-type))
	   (walker:rest-parameter
	    ;; TODO: when I have implemented algebraic types, declare types of the PARAMETER in +BUILTIN-LISP-FUNCTIONS+ like so (DECLARE (FTYPE (FUNCTION (&REST (LIST NUMBER NUMBER)) NUMBER) +)) and match the elements of FORM with the declared &REST types (i.e. (LIST NUMBER NUMBER)).
	    )
	   (walker:key-parameter (meet-non-list-type))))))))
...
(defun test-infer ()
  (flet ((assert-infer (form desired-result)
    ...
    (assert-infer '(let ((a (1+ 1))) (floor a)) '(the-ul (integer nil) (real nil)))
    (assert-infer '(let ((a 1)) a) '(the-ul (fixnum fixnum)))
    (assert-infer '(let ((a 1)) (setq a 1.0)) '(the-ul (single-float single-float)))
    ;; test IF-FORM join.
    (assert-infer '(if 1 1 1.0) '(the-ul (real real)))
    (assert-infer '(let ((a nil)) (if 1 (setq a 1.0) (setq a 1))) '(the-ul (real real)))
    (assert-infer '(let ((a nil)) (if (setq a 1) (setq a 1.0)) a) '(the-ul (real real))) ;SETQ in TEST-FORM
    ;; check that an empty else-form works.
    (assert-infer '(let ((a 1)) a (if a (setq a 1.0)) a) '(the-ul (real real)))
    ;; IF-FORM: check that joining does not join the wrong variables.
