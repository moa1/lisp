...
(defun make-orgap (genes x y angle energy)
  (labels ((compile-genes (genes)
    ...
    (multiple-value-bind (code markers) (compile-genes genes)
      (make-orgap* :genes genes :code code :markers markers :x x :y y :angle angle :energy energy :genesx genes))))
...
(defun eval-orgap (iters org orgcont add-offspring-function)
  (declare (optimize (debug 3)))
  (with-slots (genes code markers ip wait angle target targeter x y energy skin off-genes off-length as bs an bn genesx) org
