...
(defun make-orgap (genes)
  "Return compiled values for the slots (GENES CODE MARKERS FUNCTIONS GENESX)."
  (declare (optimize (debug 3)))
  (labels ((return-defaults ()
	     (return-from make-orgap (values nil #() nil nil nil))))
	...
	(let ((template genes)
	 ...
	 ;;(prind (assoc function-label markers))
	   (when (null (assoc function-label functions))
	     (return-defaults)))
      (values genes code markers functions genes))))
  ...
  (with-slots (genes code markers functions ip wait angle target targeter x y total-energy energy skin off-genes off-length as bs cs an bn stack sp genesx) org
    (let* ((max-ip (1- (array-dimension code 0)))
      ...
      (labels ((die (reason)
		 ...
		 (when (<= (xorshift:random 100) 0)
		   (setf off-energy (xorshift:random (1+ (get-energy energy 0)))))
		 ...
		 (multiple-value-bind (off-genes off-code off-markers off-functions off-genesx)
		     (make-orgap off-genes)
		   (let ((off-org (make-instance 'org :genes off-genes :code off-code :markers off-markers :functions off-functions :x off-x :y off-y :angle angle :total-energy 0 :energy off-energy :genesx off-genesx)))
	...
	(loop for i below iters do
		...
		(when (<= (get-energy energy 0) 0)
		  (die "out of energy"))
		;;(when (<= (xorshift:random 100000) 1)
		;;  (setf ip (xorshift:random (length code))))
		(incf ins-count)
		  ...
		  (macrolet ((make-instructions (keyform &body body)
			...
			(setf wait 10000)))
		     ...
		     ((set-bs-random)
		      (setf bs (random-ins)))
		     ((read-as)
		      (setf as (if (<= (xorshift:random 2000) 0) (random-ins) (car genesx))))
		     ((read-bs)
		      (setf bs (if (<= (xorshift:random 2000) 0) (random-ins) (car genesx))))
		     ((read-next)
			...
			(when (= 0 sum)
			  (setf abs-an 1 sum 2))
			(setf cs (< (xorshift:random sum) abs-an))))
		     ((mrk0=))
		     ((mrk1=))
		     ((mrk2=))
		     ((jne0=)
			...
			(if (<= 0 ip max-ip)
		     ...
		     ((set-angle-to-an)
		      (setf angle (float (/ an 128))))
		     ((set-angle-to-bn)
		      (setf angle (float (/ bn 128))))
		     ((add-to-an-bn)
			...
			(cell-division x y bn)))
		     ((turn-cw-an)
		      (incf angle (float (/ an 128)))
		      (setf wait 300))
		     ((turn-cw-bn)
		      (incf angle (float (/ bn 128)))
		      (setf wait 300))
		     ((turn-ccw-an)
		      (decf angle (float (/ an 128)))
		      (setf wait 300))
		     ((turn-ccw-bn)
		      (decf angle (float (/ bn 128)))
		      (setf wait 300))
		     ((walk-an)
		      (let* ((speed (/ an 128))
			...
			(let* ((tenergy (orgap-energy target)))
			  ...
			  (when target
			    (let* ((diff-x (- (orgap-x target) x))
			      ...
			      (setf angle (float (atan diff-y diff-x))))))))
