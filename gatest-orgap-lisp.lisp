		  ...
		  (macrolet ((make-instructions (keyform &body body)
				 ...
				 (labels
					...
					(let ((fun-name (unconditional-function-name ins)))
				      ...
				      (add-conditional-instruction (ins prefix test)
					(let* ((fun-name (unconditional-function-name ins))
					       (name (string ins))
					       (new-name (concatenate 'string prefix name))
			...
			(let ((r (random 2000)))
		     ...
		     ((set-cs-t)
		      (setf cs t))
		     ((set-cs-random)
		      (setf cs (let* ((abs-an (abs an)) (abs-bn (abs bn)) (sum (+ abs-an abs-bn))) (< (random sum) abs-an))))
		     ((mrk0))
		     ((mrk1))
		     ((mrk2))
		     ((jne0)
			...
			(if (<= 0 ip max-ip)
			    (go next-ins)
			    (die "invalid jump target"))))
		     #| doesn't work yet (fails upon call to CALL0)
		     ((func0)
