...
(defun make-orgap (genes x y angle initial-energy)
  ...
  (labels ((return-defaults ()
	     (return-from make-orgap (make-orgap* :genes nil :code #() :markers nil :functions nil :x x :y y :angle angle :energy (make-array *num-energies* :element-type 'fixnum :initial-element 0) :genesx nil))))
	...
	(incf (aref energy 0) initial-energy)
	(make-orgap* :genes genes :code code :markers markers :functions functions :x x :y y :angle angle :total-energy 0 :energy energy :genesx genes)))))
  ...
  (with-slots (genes code markers functions ip wait angle target targeter x y total-energy energy skin off-genes off-length as bs cs an bn stack sp genesx) org
      ...
      (labels ((die (reason)
		 ...
		 (let ((x (floor x))
		   ...
		   (loop for energy-index below *num-energies* do
			(incf (aref *world* x y energy-index) (max (aref energy energy-index) 0))
			(setf (aref energy energy-index) 0)))
		 ...
		 (when (<= (random 100) 0)
		   (setf off-energy (random (1+ (aref energy 0)))))
		 (setf off-genes (nreverse off-genes))
		 (setf off-energy (clamp off-energy 0 (aref energy 0)))
		 ...
		 (let ((off-orgap (make-orgap off-genes off-x off-y angle off-energy)))
		   ...
		   (let ((off-orgcont (funcall add-offspring-function off-orgap)))
		     (setf (aref (orgcont-energy-in-sum off-orgcont) 0) off-energy))
		   (incf wait 20000))
		 (decf (aref energy 0) off-energy) (incf (aref (orgcont-energy-out-sum orgcont) 0) off-energy)
	...
	(loop for i below iters do
		...
		(let* ((ins (aref code ip)))
		  ...
		  (decf (aref energy 0)) (incf (aref (orgcont-energy-out-sum orgcont) 0))
		  (macrolet ((make-instructions (keyform &body body)
				 ...
				 (labels
				   ...
				   (let* ((instructions-list (mapcar #'caar cases))
				     ...
				     (setf (gethash nil instructions-hash-table) 0)
		    ...
		    (make-instructions
		     ...
		     ((eat)
		      (let* ((x (floor x))
			     (y (floor y)))
			;;(loop for energy-index below *num-energies* do
			(loop for energy-index below 1 do
			     (let* ((energy-available (floor (aref *world* x y energy-index)))
				    (bite-size (ash energy-available -2))
				    (meal (min (- *org-max-total-energy* total-energy) bite-size)))
			       (incf (aref energy energy-index) meal)
			       (incf total-energy meal)
			       (incf (aref (orgcont-energy-in-sum orgcont) energy-index) meal)
			       (decf (aref *world* x y energy-index) meal)))
			(setf wait 10000)))
		     ((set-as-nil)
		      (setf as nil))
		     ((set-bs-nil)
			...
			(if (<= 0 ip max-ip)
		     ...
		     ((set-bn-to-an)
		      (setf bn an))
		     ((set-an-to-energy)
		      (setf an (aref energy 0)))
		     ((set-bn-to-energy)
		      (setf bn (aref energy 0)))
		     ((set-an-to-angle)
			...
			(setf bn (floor (aref *world* (floor x) (floor y) ins-index)))))
		     ...
		     ((set-bn-max-an-bn)
		      (setf bn (max an bn)))
		     ((split-cell-an)
		      (when (and (>= an 0) (>= (aref energy 0) an))
			(cell-division x y an)))
		     ((split-cell-bn)
		      (when (and (>= bn 0) (>= (aref energy 0) bn))
			...
			(when (>= (aref *world* (floor new-x) (floor new-y) 0) 0)
			  (let ((cost (max 0 (floor (expt (* (abs speed) 16) 2)))))
			    (decf (aref energy 0) cost) (incf (aref (orgcont-energy-out-sum orgcont) 0) cost))
