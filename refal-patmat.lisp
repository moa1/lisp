...
Doesn't yet print whether dll has any circularities in it (but detects them alre
ady)."
	  ...
	  (symbol ;;(prind (var-type pat vars))
		  (case (var-type pat vars)
		    ...
		    ((evar) (if (eq pat-l pat-r)
				...
				(values (let* ((val (if (eq exp-l exp-r)
							(if (dlistp exp) (dlist->list* exp) exp)
							(dcons->list exp-l exp-r))))
	...
	(let* ((pat (data pat-l))
	  ...
	  (flet ((try (val exp-l-next)
	    ...
	    (do ((cur exp-l (next cur))) (())
	      (let* ((val (if (eq exp-l cur)
			      (let ((d (data exp-l)))
				(if (dlistp d) (dlist->list* d) d))
			      (dcons->list exp-l cur)))
