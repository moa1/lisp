...
;;
;; I have the following image in my mind: Let's call each of the defining components of a configuration C, that is, starting state, variables, states of variables, and (nested?) state machines, a "dimension". Then we could visualize the exploration of the universe by running several configurations by drawing a line between successive steps of a configuration. What happens when you run the configurations for a while is that there are "worms" that squirm through the space of possible dimension states. When a worm (i.e. a configuration) follows another worm (i.e. another configuration) for too long, then it gets replaced by another configuration by altering the replaced configuration slightly. The event "configuration x follows configurtion y for too long" takes place if d_sequence(Qx,Qy) < threshold1.
;;
;; Likewise, we could stop running or eliminate a configuration if it becomes too lonely, i.e. there are no other configurations that are nearby. (I.e. d_sequence(Qx,Qy) > threshold2.) This has the effect that the universe is not explored too sparsely, which will be likely when it becomes too large.
;;
;; (I think d_sequence(Qx,Qy) should take into account time shifts between Qx and Qy. For example, Qx and Qy could be identical sequences, but shifted by an offset. If we don't take into account that Qx and Qy might just be shifted sequences, d_sequences computes a value which doesn't reflect their similarity. Maybe use a multidimensional FFT or something like that? And abort when the spectrums become too similar? (Ignore the phases of the spectrum, but just compare the amplitude spectrum?)
